...
class Ticket extends Conectar{
	...
	/*** para el detalle de tickets ***/
	public function listar_ticketdetalle_x_ticket($tick_id){
		$conectar = parent::conexion();
		parent::set_names();
		$sql= "SELECT 
					  td_ticketdetalle.ticked_id,
					  td_ticketdetalle.ticked_descrip,
					  td_ticketdetalle.fecth_crea,
					  tm_usuario.usu_nom,
					  tm_usuario.usu_ape,
					  tm_usuario.rol_id
					  FROM
    ...
    public function insert_ticketdetalle($tick_id,$usu_id,$tickd_descrip){
            $conectar= parent::conexion();
            parent::set_names();
                $sql="INSERT INTO td_ticketdetalle (ticked_id,tick_id,usu_id,ticked_descrip,fecth_crea,est) VALUES (NULL,?,?,?,now(),'1');";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $tick_id);
    ...
    public function insert_ticketdetalle_cerrar($tick_id,$usu_id){
            $conectar= parent::conexion();
            parent::set_names();
                $sql="INSERT INTO td_ticketdetalle (ticked_id,tick_id,usu_id,ticked_descrip,fecth_crea,est) VALUES (NULL,?,?,'Ticket Cerrado',now(),'1');";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $tick_id);
            $sql->bindValue(2, $usu_id);
            $sql->execute();
